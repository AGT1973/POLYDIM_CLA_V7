...
def writer_worker(tag, iters):
    pmtp = PMTPSlabChannel(tag, 16, True)
    print(f"Writer started on tag {tag}")
    # Write a perfectly orthogonal matrix repeatedly
    # D=10^6 is too big for a quick test, we will use D=16 for speed so we can do many iterations
    tensor = np.eye(16, dtype=np.float64)
    for i in range(iters):
        # Mutate the tensor slightly but keep it orthogonal (just flip signs or something)
        # We'll just write eye(16) * (-1)**i
        mutated = tensor * (1 if i % 2 == 0 else -1)
        pmtp.write_tensor(mutated)
    print("Writer finished.")

def reader_worker(tag, iters, reader_id):
    pmtp = PMTPSlabChannel(tag, 16, False)
    success = 0
    torn = 0
    race_caught = 0
    
    start = time.time()
    # Read wildly while writer is writing
    for i in range(iters):
        try:
            tensor = pmtp.read_tensor()
            if tensor is not None:
                # Check orthogonality
                I_check = tensor.T @ tensor

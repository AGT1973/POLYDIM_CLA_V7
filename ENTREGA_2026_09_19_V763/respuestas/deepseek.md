...
***But there is a critical bug in `polydim\_stiefel\_cayley\_smw\_f64` final V763: After Parche B, they update S for XtG and GtG, but they do NOT update `XtX`? Not needed. But they also use `XtG` in M construction for r\<K, c\>=K? That's XtX. For r\>=K, c\<K: -GtG. For r\>=K, c\>=K: -XtG(c-K, r-K). That uses projected XtG. Good. But wait `GtG` lambda reads S\[(K+r)K2 + (K+c)\]. They recompute GtG\_proj and store there. Good. But `XtG` lambda reads S\[r*K2 + (K+c)\]. They updated that. Good.**
...
***Thus, final PMTP is not matching contract and not SPMC safe. The V762 implementation in monolith was correct. So the final V763 kernel has regressed PMTP.**
...
***Another major issue: In final `polydim\_kernel.cpp`, the function `polydim\_rodrigues\_geodesic\_f64` signature has `\_\_restrict\_\_` in both header and .cpp. But in header, `polydim.h` V763 has `\_\_restrict\_\_` for all functions. However, `\_\_restrict\_\_` is not standard C. In C++ it's an extension. If compiling with MSVC, it might fail. They claim MSVC 2026 support. Should define macro `POLYDIM\_RESTRICT` to `\_\_restrict` on MSVC and `\_\_restrict\_\_` on GCC/Clang, or use `restrict`? But `restrict` is C99 not C++. So need portability fix.**

***Also in `polydim.h` V763, the include guard is `POLYDIM\_H`. But the monolith V762 header also uses `POLYDIM\_H`. If both included, conflict. Not relevant.**

***Now, the Python monolith `polydim\_v763\_monolito.py` (and v762) has inconsistencies with C++ symbols. In `polydim\_v763\_monolito.py`, `PolydimNativeCore.\_\_init\_\_` tries to load `polydim\_apply\_rodrigues\_geodesic\_f64` and `polydim\_stiefel\_cayley\_smw\_retraction\_f64`, but the actual C++ exports are `polydim\_rodrigues\_geodesic\_f64` and `polydim\_stiefel\_cayley\_smw\_f64`. Also it tries to load `compute\_gram\_and\_factorize` which is not exported. And Rust `polydim\_rust\_verify\_invariants` signature in Python is wrong: in Rust it's `(y, u, v, d, \*mut VerifyReport)`, but Python v763 uses `argtypes = \[c\_void\_p, c\_void\_p, c\_void\_p, c\_size\_t, POINTER(VerifyReport)\]` and calls with `(tensor.ctypes.data, None, None, D, byref(report))`. That matches 5 args. In v762 monolith, it used 3 args and wrong. So v763 Python is better but still has wrong C++ function names. Also `verify\_betti1` calls `polydim\_rust\_betti1\_guard` but Rust code no longer implements it (A8 removed). So that will fail. Also `PMTPSlabChannel` is a 2-buffer Python implementation, not the triple buffer C++ PMTP. It is inconsistent with the C++ PMTP API. It uses `mmap` and sequence packed with 1 bit buffer index, 2 buffers. That's the old vulnerable 2-buffer. So the Python PMTP is not the hardened PMTP.**

***Also `test\_v763\_mpeleides.py` builds binaries from `kernel\_cpp\_v762.cpp` and `kernel\_rust\_v762.rs` in `ENTREGA\_DIR`, but the provided files are `polydim\_kernel.cpp` and `lib.rs`? The filenames mismatch. It then imports `from polydim\_v762\_monolito import PolydimNativeCore, PMTPSlabChannel`. But the provided monolith is `polydim\_v763\_monolito.py`. So test script is outdated. It also expects `apply\_rodrigues\_geodesic` etc. But the C++ exports are different. So the test script will fail to compile/import. It also asserts in SUITE 4 that `verify\_betti1` returns -6 for fragmented graph. But Rust code removed Betti-1. So that test will fail. Also in SUITE 5, Attack 4 expects Rust status -4 for subnormal, but Rust code treats subnormal as success (0) and counts it. So that test will fail. So the test suite is inconsistent with the final code.**

***Also `polydim\_ffi\_v763.dart.txt` is a different Dart bridge that uses old symbols `polydim\_apply\_rodrigues\_geodesic\_f64`, `polydim\_rust\_verify\_invariants` with 3 args, `polydim\_rust\_betti1\_guard`. This does not match final C++/Rust exports. The final Dart bridge `polydim\_ffi.dart` uses correct `polydim\_rodrigues\_geodesic\_f64` and 8-arg report, and does not use Betti. So the v763 dart file is outdated and wrong.**

***The Triton kernel `polydim\_triton\_kernel\_v763.py` and v762: It computes partial dot products in pass1, then sums on CPU, computes alpha/beta, then pass2. But it does not validate orthonormality or point norm. It also uses `torch.sin(torch.tensor(...))` which is inefficient and may not be deterministic. But more importantly, it does not match the C++ Rodrigues formula? Let's check alpha/beta. C++:**
...
***Now, the `02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md` contains V762 code that is more correct. The final `polydim\_kernel.cpp` (V763) introduced changes that broke PMTP and added internal projection. The user wants to finish \<2000 lines perfect. We need list all errors and solutions. We should be exhaustive but not hallucinate. We have many files. We need identify discrepancies between versions and which is authoritative. The latest files seem to be `polydim.h`, `polydim\_kernel.cpp`, `lib.rs`, `polydim\_ffi.dart`, `polydim\_v763\_monolito.py`, `test\_v763\_mpeleides.py`, `polydim\_triton\_kernel\_v763.py`, `polydim\_ffi\_v763.dart.txt`. But many are inconsistent. The user probably wants us to audit the final code and give solutions to make it perfect. We need point out that there are multiple version mismatches, outdated test scripts, wrong symbol names, PMTP regression, Betti-1 removal inconsistency, subnormal handling inconsistency, etc.**
...
- ***polydim\_ffi\_v763.dart.txt (outdated)**

- ***polydim\_triton\_kernel\_v763.py**

- ***polydim\_v763\_monolito.py**

- ***readme\_first.md**

- ***test\_v763\_mpeleides.py**
...
- ***kernel\_cpp\_v763.cpp.txt (same as polydim\_kernel.cpp? It is V762 header comment but code V762? Actually kernel\_cpp\_v763.cpp.txt content is the V762 kernel from monolith, not the final V763 with Parche B. Let's check: It starts with "POLYDIM V762 — KERNEL ENDURECIDO" and PMTP triple buffer seqlock. Yes, it's the V762 kernel. The final `polydim\_kernel.cpp` is V763 with Parche B and broken PMTP. So there are two C++ kernels: `kernel\_cpp\_v763.cpp.txt` is actually V762 code (despite filename v763), and `polydim\_kernel.cpp` is V763 with changes. The user may be confused. We need clarify.**

- ***kernel\_rust\_v763.rs.txt is V762 Rust (same as lib.rs? It has Betti-1 removed, same as lib.rs). So Rust is consistent.**

- ***polydim\_kernel.cpp final V763.**

- ***lib.rs final Rust (same as kernel\_rust\_v763.rs.txt).**

- ***bench.cpp, bench\_sphere.cpp, test\_suite.cpp, polydim.h final V763.**
...
3. ***Betti-1 removal inconsistency:**
   ...
   - ***But Python `polydim\_v763\_monolito.py` still has `verify\_betti1` calling `polydim\_rust\_betti1\_guard`.**

   - ***`test\_v763\_mpeleides.py` SUITE 4 tests Betti-1 and expects -6.  
...
4. ***Subnormal handling inconsistency:**
   ...
   - ***`test\_v763\_mpeleides.py` Attack 4 expects Rust status -4 (errSubnormalDetected).  
...
5. ***Dart FFI outdated:**
   ...
   - ***`polydim\_ffi\_v763.dart.txt` uses old signatures and Betti. Solution: Delete or replace with final `polydim\_ffi.dart`. The final Dart is correct.**
...
8. ***C++ header/version inconsistency:**
   ...
   - ***`polydim.h` says V763, `polydim\_kernel.cpp` build\_info says "POLYDIM V763", but many files say V762. Solution: Choose one version (V763) and update all comments, build\_info, docs, file names. Or revert to V762 if V763 changes are not ready.**
...
20. ***\*\*Python `PolydimNativeCore` in v763 sets `self.cpp\_lib.polydim\_rodrigues\_geodesic\_f64.argtypes` with 8 args, but the actual C++ function signature has 8 args: y,u,v,y\_out,theta,D,tol,report. So that matches. But it calls with `None` for tol and report, which is okay. However, it still defines `apply\_rodrigues\_geodesic` calling `polydim\_apply\_rodrigues\_geodesic\_f64` which doesn't exist. So the method is broken. Solution: Rename to `polydim\_rodrigues\_geodesic\_f64` and update argtypes. Same for Stiefel.**

21. ***\*\*Python `test\_v763\_mpeleides.py` builds from `kernel\_cpp\_v762.cpp` and `kernel\_rust\_v762.rs` but those files are not in the provided list. The actual files are `polydim\_kernel.cpp` and `lib.rs`. Solution: Update paths.**

22. ***\*\*Python `test\_v763\_mpeleides.py` imports `polydim\_v762\_monolito` but file is `polydim\_v763\_monolito.py`. Solution: Update import.**

23. ***\*\*Python `test\_v763\_mpeleides.py` SUITE 5 Attack 3 expects Rust status -8 for zero vector. But Rust `polydim\_rust\_verify\_invariants` returns `POLYDIM\_ERR\_DEGENERATE\_NORM = -4` for yy \<= 0. So expected -8 is wrong. Solution: Update expected to -4.**

24. ***\*\*Python `test\_v763\_mpeleides.py` SUITE 5 Attack 4 expects -4 for subnormal. Rust returns 0. Update.**

25. ***\*\*Python `test\_v763\_mpeleides.py` SUITE 4 expects Betti-1. Rust has no Betti. Update or remove.**
...
53. ***C++ `polydim\_pmtp\_commit\_write`: It does not check if slot is valid (0..2). It should. In V762, it checks `slot \>= POLYDIM\_PMTP\_SLOTS`. In V763, it doesn't. Solution: Add check.**
...
56. ***C++ `polydim\_build\_info`: Returns "POLYDIM V763". But other files say V762. Solution: Unify.**

57. ***C++ `polydim.h`: The comment says "POLYDIM V763 — CONTRATO PÚBLICO". But the enum has comments "nuevos en V762". Inconsistent.**

58. ***C++ `polydim\_kernel.cpp`: The top comment says "POLYDIM V763 — KERNEL ENDURECIDO" but then lists A1..A12. It also has "Parche B (V763): Proyeccion al espacio tangente ANTES del solver". This is a new change. But the PMTP implementation is completely different and not documented. So the file is a mix.**
...
81. ***C++ `polydim\_build\_info`: It returns "POLYDIM V763". Good if V763 is intended.**

82. ***Python `polydim\_v763\_monolito.py`: It has `PolydimNativeCore` which sets argtypes for `polydim\_rodrigues\_geodesic\_f64` with 8 args. But the C++ function has 8 args: y, u, v, y\_out, theta, D, tol, report. The Python call:**
...
***This matches. Good. But the method is named `apply\_rodrigues\_geodesic` and it calls the correct symbol. Wait in v763, the code:**
...
***So it uses the correct symbol! In v762 monolith, it used `polydim\_apply\_rodrigues\_geodesic\_f64`. So v763 Python is correct for Rodrigues. Let's check Stiefel:**
...
***So v763 Python uses correct symbols. Good. The v762 monolith had wrong symbols. So v763 Python is better. But it still has `compute\_gram\_and\_factorize`? In v763, I don't see it. Let's re-read v763 Python: It defines `PolydimNativeCore` with `\_\_init\_\_` setting `polydim\_rodrigues\_geodesic\_f64`, `polydim\_stiefel\_cayley\_smw\_f64`, `polydim\_selftest\_all`, `polydim\_build\_info`, `polydim\_rust\_verify\_invariants`. It does NOT have `compute\_gram\_and\_factorize`. Good. So v763 Python is updated. The v762 monolith had wrong symbols. So we should focus on v763 as final. The v763 Python still has `verify\_betti1` calling `polydim\_rust\_betti1\_guard`, which is not in Rust. So that's a bug. Also `PMTPSlabChannel` is 2-buffer. Also `test\_v763\_mpeleides.py` uses `polydim\_v762\_monolito` and wrong expected codes. So those are bugs.**

83. ***Python `polydim\_v763\_monolito.py`: `PolydimNativeCore.\_\_init\_\_` calls `self.cpp\_lib.polydim\_selftest\_all()` and raises if rc != 0. Good. But it does not set argtypes/restype for `polydim\_build\_info`? It does: `self.cpp\_lib.polydim\_build\_info.argtypes = \[\]`, `restype = c\_char\_p`. Good.**

84. ***Python `polydim\_v763\_monolito.py`: `verify\_rust\_invariants` calls Rust with `(tensor.ctypes.data, None, None, D, byref(report))`. The Rust function expects `(y, u, v, d, report\_out)`. So passing None for u and v is correct (null pointers). Good.**

85. ***Python `polydim\_v763\_monolito.py`: `verify\_betti1` is dead code but will fail if called. Should be removed.**

86. ***Python `polydim\_v763\_monolito.py`: `PMTPSlabChannel` is not used in `PolydimNativeCore`. It's a separate class. It might be used in tests. The test uses `PMTPSlabChannel` from `polydim\_v762\_monolito` (wrong import). In v763, it's in `polydim\_v763\_monolito`. So test should import from v763. But the class itself is 2-buffer and not hardened. If the test uses it, it won't test the C++ PMTP. The test SUITE 2 uses `PMTPSlabChannel` for shared memory round-trip, not for concurrency. So it might pass. But it's not the hardened PMTP. The concurrency test A1 is in C++ test\_suite.cpp. So Python PMTP is just for IPC, not for the seqlock. Still, it should be updated to triple buffer or clearly documented as separate.**

87. ***Python `test\_v763\_mpeleides.py`: It builds `CPP\_SRC = os.path.join(ENTREGA\_DIR, "kernel\_cpp\_v762.cpp")` but the file is `polydim\_kernel.cpp`. So build fails. Solution: Update to `polydim\_kernel.cpp` and `lib.rs`.**

88. ***Python `test\_v763\_mpeleides.py`: It compiles Rust with `RUST\_SRC = os.path.join(ENTREGA\_DIR, "kernel\_rust\_v762.rs")` but file is `lib.rs`. Update.**

89. ***Python `test\_v763\_mpeleides.py`: It imports `from polydim\_v762\_monolito import PolydimNativeCore, PMTPSlabChannel`. Update to `polydim\_v763\_monolito`.**

90. ***Python `test\_v763\_mpeleides.py`: SUITE 1 asserts `rust\_drift \< 1e-10`. Rust bound is 1.42e-14, so fine.**

91. ***Python `test\_v763\_mpeleides.py`: SUITE 3 uses `core.apply\_stiefel\_retraction(X, G, 0.1)`. In v763 Python, method is `apply\_stiefel\_retraction`. It calls `polydim\_stiefel\_cayley\_smw\_f64`. Good.**

92. ***Python `test\_v763\_mpeleides.py`: SUITE 3 asserts `stiefel\_drift \< 1e-8`. Fine.**

93. ***Python `test\_v763\_mpeleides.py`: SUITE 4 expects Betti-1. Rust has no Betti. So `core.verify\_betti1` will call missing symbol and crash. Update.**

94. ***Python `test\_v763\_mpeleides.py`: SUITE 5 Attack 3 expects Rust status -8 for zero vector. Rust returns -4 (DEGENERATE\_NORM). Update.**

95. ***Python `test\_v763\_mpeleides.py`: SUITE 5 Attack 4 expects Rust status -4 for subnormal. Rust returns 0. Update.**
...
107. ***C++ `test\_suite.cpp`: It uses `polydim\_pmtp\_\*` functions. With final V763 PMTP, the test A1 may pass but does not test seqlock. The test checks `races` but `validate` always returns success, so races=0. The test prints but does not assert races\>0. It asserts `ud==0` and `ok\>0`. With state machine, `ud` should be 0 if no torn reads. `ok` should be \>0. So test passes. But it's not testing the seqlock. The test name says "la prueba que en V761 dio 99,65 % de lecturas desgarradas". In V763, it's not testing that. So the test is misleading. Solution: Either revert PMTP to V762 and keep test, or update test to reflect new PMTP and remove seqlock claims.**

108. ***C++ `test\_suite.cpp`: It checks `check(true, "POLYDIM\_ERR\_SEQLOCK\_RACE tiene ruta de retorno", b4);` This always passes regardless of races. That's a useless test. Should assert `races \> 0` if seqlock is expected. With V763 PMTP, races=0, so it would fail if asserted. So V763 PMTP breaks the test's intent.**

109. ***C++ `bench.cpp`: It uses `polydim\_stiefel\_cayley\_smw\_f64` with `Gt` from `polydim\_project\_tangent\_stiefel\_f64`. With V763 internal projection, it double projects. That's fine but extra work. The benchmark may show V763 slower than V762 due to internal projection. The README claims V762 performance. If V763 is intended, benchmarks should be updated.**
...
117. ***Python `test\_v763\_mpeleides.py`: Attack 1 expects C++ status -3 for NaN. C++ `POLYDIM\_ERR\_NAN\_OR\_INF = -3`. Good. Rust status -3. Good.**

118. ***Python `test\_v763\_mpeleides.py`: Attack 2 expects C++ status -3 for Inf. Good.**

119. ***Python `test\_v763\_mpeleides.py`: Attack 3 expects Rust status -8. Should be -4. Update.**

120. ***Python `test\_v763\_mpeleides.py`: Attack 4 expects Rust status -4. Should be 0 (success) and subnormal\_count=1. Update.**

121. ***Python `test\_v763\_mpeleides.py`: SUITE 4 expects `verify\_betti1` returns 0 for connected, -6 for fragmented. Rust has no such function. If we remove Betti, delete SUITE 4.**

122. ***Python `test\_v763\_mpeleides.py`: It uses `core.apply\_stiefel\_retraction` which calls `polydim\_stiefel\_cayley\_smw\_f64`. The C++ function now internally projects G. The test passes G as small random. It does not project G before. So internal projection will project it. That's fine. But the test does not call `polydim\_project\_tangent\_stiefel\_f64`. So internal projection is necessary for the test to be correct. In V762, the test would have failed because G not tangent. So V763 internal projection fixes that. So Parche B is actually useful. The test SUITE 3 in v763 passes G without projection. So internal projection is needed. Good. So we should keep Parche B, but optimize memory.**

123. ***Python `test\_v763\_mpeleides.py`: SUITE 3 asserts `stiefel\_drift \< 1e-8`. With internal projection, output orthonormality should be good. Fine.**

124. ***Python `test\_v763\_mpeleides.py`: SUITE 1 uses `core.apply\_rodrigues\_geodesic` which calls `polydim\_rodrigues\_geodesic\_f64`. It passes y, u, v, theta. The C++ function checks y norm and basis orthonormality. The test constructs y by normalizing random, u by normalizing random, v by Gram-Schmidt. But the Gram-Schmidt in test is not compensated. For D=1e6, the naive Gram-Schmidt may leave residual \> 64\*eps. Let's check: In SUITE 1, they do:**
...
125. ***Python `test\_v763\_mpeleides.py`: SUITE 1 uses `np.linalg.norm(y)` to normalize. For D=1e6, this is not compensated. The C++ kernel checks `e\_yy = |\<y,y\>-1|`. For a vector normalized by numpy, the error might be around 1e-16? Actually np.linalg.norm computes sqrt of sum of squares. The sum of squares of a random unit vector is ~1. The error in sum is ~eps \* sqrt(D) ≈ 2e-13. Then sqrt(1+δ) ≈ 1+δ/2. So \<y,y\> = 1+δ, error ~2e-13. The C++ gate `point\_norm = 64\*eps = 1.42e-14`. So 2e-13 \> 1.42e-14. The kernel will reject with POINT\_OFF\_MANIFOLD. So SUITE 1 will fail even on y. So the test is fundamentally broken because it doesn't use the compensated projection. Solution: Use `core.cpp\_lib.polydim\_project\_sphere\_f64` to normalize y, and `polydim\_orthonormalize\_pair\_f64` to build basis. The Dart example does this. The Python test must be updated.**

126. ***Python `test\_v763\_mpeleides.py`: SUITE 3 uses `np.linalg.qr` to orthonormalize X. QR is not compensated. For D=1e6, K=8, the X^T X error may be ~1e-13. The C++ gate `gram\_ortho = 64\*eps\*sqrt(D) = 1.42e-14 \* 1000 = 1.42e-11`. So 1e-13 \< 1.42e-11, so it passes. So SUITE 3 is okay because Stiefel gate is lax. But SUITE 1 uses sphere gate which is strict. So SUITE 1 fails.**

127. ***Python `test\_v763\_mpeleides.py`: SUITE 1 also does `v = v - np.dot(u, v) \* u; v /= np.linalg.norm(v)`. The resulting |\<u,v\>| may be ~1e-13. The sphere basis gate is 1.42e-14. So fails. So SUITE 1 definitely fails. This is a critical bug in the test suite. The user wants to finish \<2000 lines perfect. The test must be fixed to use the library's compensated routines.**

128. ***Python `test\_v763\_mpeleides.py`: It imports `PMTPSlabChannel` from `polydim\_v762\_monolito`. If we update to v763, the class is still 2-buffer. But SUITE 2 only does a single write/read round-trip, not concurrency. So it will pass. But it's not testing the hardened PMTP. The hardened PMTP is C++ only. So SUITE 2 is not testing A1. The A1 test is in C++ test\_suite.cpp. So Python test doesn't test concurrency. That's okay, but the file name says "ADVERSARIAL SILICON VALIDATION". It should test the C++ PMTP via FFI, not Python mmap. But the C++ PMTP API is not exposed in Python. So Python test cannot test A1. Solution: Either expose PMTP via FFI or remove SUITE 2 and rely on C++ test.**

129. ***Python `test\_v763\_mpeleides.py`: It builds binaries with `-static-libgcc -static-libstdc++`. On Windows MinGW, that's okay. But it uses `GCC\_PATH = r"E:\\winlibs\_gcc14\_zip\\mingw64\\bin\\g++.exe"`. Hardcoded path. Not portable. Solution: Use environment variable or `g++` from PATH.**

130. ***Python `test\_v763\_mpeleides.py`: It uses `RUSTC\_PATH = r"C:\\Users\\eluithi\\.cargo\\bin\\rustc.exe"`. Hardcoded. Not portable. Solution: Use `rustc` from PATH.**

131. ***Python `polydim\_v763\_monolito.py`: It has hardcoded `mingw\_bin = r"E:\\winlibs\_gcc14\_zip\\mingw64\\bin"`. Not portable. Solution: Use environment variable or search.**
...
135. ***C++ `polydim\_kernel.cpp`: `polydim\_pmtp\_acquire\_read` returns middle slot. It does not use `observed\_seq` to check for new data. It relies on flag bit. But flag is set by writer commit and cleared by reader acquire. So if multiple readers, first reader clears flag, others see no data. That's not SPMC. For single reader, it works. But the header says SPMC? Actually V762 header says "triple búfer con seqlock por ranura" and "Invariante: la ranura en escritura nunca es la ranura publicada, y el lector detecta cualquier solapamiento vía POLYDIM\_ERR\_SEQLOCK\_RACE." It doesn't explicitly say SPMC in header, but README says "zero-tear lock-free SPMC concurrency". So it should support multiple readers. V763 does not. So critical.**

136. ***C++ `polydim\_kernel.cpp`: The PMTP state machine is not lock-free? It uses CAS, so lock-free. But multiple readers mutate shared state, so they are not independent. A correct SPMC triple buffer with seqlocks has readers not mutating writer state except their local observed\_seq. The V762 design does that. So V763 is a regression.**

137. ***C++ `polydim\_kernel.cpp`: The `PMTP\_Control` struct in V763 is only 64 bytes. The V762 was larger with 3 slot\_seq, global\_seq, write\_cursor. The V762 design allowed per-slot seqlocks and multiple readers. So V763 lost functionality.**

138. ***C++ `polydim\_kernel.cpp`: The `polydim\_pmtp\_validate\_read` in V763 ignores `slot` and `ticket`. It always returns success. So even if reader copies a slot that writer is writing, it won't detect. In V762, it compares ticket. So V763 is unsafe.**
...
143. ***C++ `polydim\_kernel.cpp`: The `polydim\_pmtp\_acquire\_read` clears flag bit. So after one reader reads, flag=0. Another reader will get no new data. So multiple readers cannot each get the same data. That's a broadcast problem. SPMC should allow multiple consumers to each read the latest value. V762 design with global\_seq and per-reader observed\_seq allows that. V763 does not. So critical.**

144. ***C++ `polydim\_kernel.cpp`: The `polydim\_pmtp\_validate\_read` always success. So no race detection.**

***Thus, the PMTP V763 is completely broken for SPMC and seqlock. The solution is to revert to the V762 PMTP implementation from `02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md` (or `kernel\_cpp\_v763.cpp.txt` which actually contains V762 PMTP). That implementation is correct. The V763 changes to PMTP should be discarded.**

145. ***C++ `polydim.h`: The header comment says "POLYDIM V763 — CONTRATO PÚBLICO". But the enum comments say "nuevos en V762". Should update to V763 or revert version.**

146. ***C++ `polydim\_kernel.cpp`: The top comment says "POLYDIM V763 — KERNEL ENDURECIDO". But the file includes "Parche B (V763)". So it's V763. The PMTP implementation in V763 is different. So if we revert PMTP to V762, we need to update comments.**

147. ***C++ `polydim\_kernel.cpp`: The function `polydim\_stiefel\_cayley\_smw\_f64` now has internal projection. This is a V763 feature. It should be documented in `polydim.h` and README. The header still says "G: tangent vector". It doesn't mention internal projection. Solution: Update header comment to say G is projected internally if not tangent, or require caller to project (and remove internal projection). Given the test SUITE 3 in Python does not project, internal projection is useful. But it changes API. Should document.**
...
174. ***C++ `polydim\_kernel.cpp`: The top comment says "Mejoras P1: ...". It should mention V763 changes.**

175. ***Rust `lib.rs`: The file name in `test\_v763\_mpeleides.py` is `kernel\_rust\_v762.rs`. Should be `lib.rs` or `kernel\_rust\_v763.rs`. Update.**
...
179. ***Rust `lib.rs`: The `VerifyReport` struct has `bound\_used`. The Python v763 `VerifyReport` matches. Good.**
...
190. ***C++ `test\_suite.cpp`: The A1 PMTP test with V763 PMTP will pass but not test seqlock. If we revert PMTP to V762, the test will actually test seqlock and should pass. The V762 PMTP implementation in `02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md` is correct. So we should revert PMTP to V762 and keep the test. That will give real seqlock testing. So the solution is: replace V763 PMTP implementation with V762 PMTP implementation. The V762 PMTP code is in `02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md` section 2, and also in `kernel\_cpp\_v763.cpp.txt` (which is actually V762 code). So we can copy that.**
...
192. ***C++ `polydim.h`: The header comment for PMTP says "triple búfer con seqlock por ranura". That matches V762. So V763 PMTP implementation contradicts header. Revert.**
...
200. ***C++ `polydim\_kernel.cpp`: The V763 PMTP also has `polydim\_pmtp\_sizeof` and alignof. The V762 PMTP struct is larger. The test suite allocates `ctrl\_mem` using `polydim\_pmtp\_sizeof() + polydim\_pmtp\_alignof()`. That works for both. So swapping implementation is transparent.**
...
1. ***Resumen ejecutivo: Hay tres versiones mezcladas (V762, V763, V76x). El código final no es coherente. Los errores más graves: PMTP V763 roto, tests Python inválidos, FFI desactualizado, `\_\_restrict\_\_` UB, memoria en Stiefel, Betti-1 inconsistente, subnormal inconsistente.**

2. ***Hallazgos críticos (P0):**

   - ***PMTP V763 no es triple buffer con seqlock, no SPMC, validate siempre success. Solución: reemplazar por implementación V762 de `02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md` sección 2 (o `kernel\_cpp\_v763.cpp.txt`). Proveer el código exacto o indicar.**
   ...
   - ***Tests Python `test\_v763\_mpeleides.py` usan normalización numpy no compensada, fallarán en D=1e6 por tolerancias estrictas. Solución: usar `polydim\_project\_sphere\_f64` y `polydim\_orthonormalize\_pair\_f64` vía FFI para preparar inputs, como hace Dart.**
   ...
   - ***Nombres de archivos/imports en test Python: `kernel\_cpp\_v762.cpp` -\> `polydim\_kernel.cpp`; `kernel\_rust\_v762.rs` -\> `lib.rs`; `polydim\_v762\_monolito` -\> `polydim\_v763\_monolito`.**

   - ***Dart default library name `polydim.dll` vs build `libpolydim.dll`. Solución: unificar.**

   - ***Stiefel V763 asigna `G\_proj` D\*K y lo recomputa; riesgo OOM y coste. Solución: usar fórmula matricial para XtG\_proj y GtG\_proj sin materializar G\_proj. Dar fórmulas y patch.**

   - ***`polydim.h` y `polydim\_kernel.cpp` usan `\_\_restrict\_\_` que no es portable a MSVC. Solución: macro `POLYDIM\_RESTRICT`.**

   - ***Inconsistencia de versiones: V762 vs V763. Solución: decidir V763 y actualizar todo, o revertir a V762. Recomiendo V763 con PMTP V762 y Parche B optimizado.**
...
3. ***Hallazgos altos (P1):**
   ...
   - ***`polydim\_v763\_monolito.py` tiene `verify\_betti1` muerto. Eliminar.**
...
```
***/\* PMTP V763: Ring buffer SPMC lossy con cursores independientes por lector.**
...
***\# CI POLYDIM V763 — Verificación completa**
...
```
***\# Plan de Certificación Industrial POLYDIM V763**
 ...
 ***\* PMTP V763 — Ring buffer SPMC lossy.**
...
***\# CI POLYDIM V763 — Verificación completa de UB, races, ABI y reproducibilidad.**
...
***Actually the code in V763 is clearly a different design from V762. In V763, `begin\_write` returns `(s \>\> 4) & 3` (the newest slot), then `commit\_write` rotates the state so that middle becomes newest. But that doesn't match the semantics of "the writer just wrote to newest, so newest should be preserved".**
...
***Actually let me look again. In V763's `begin\_write`:**
...
***Hmm, this V763 PMTP is definitely wrong. It's a degraded version of V762's.**

***OK, so my BUG-1 is real (unused slot param in commit), and there's also a larger structural issue: the V763 PMTP state machine is fundamentally broken because begin\_write returns the currently-published slot, not a free slot. That's a bigger bug than I listed. Let me call it BUG-1b.**
...
1. ***PMTP V763 is broken (begin\_write returns published slot, commit ignores slot, validate\_read always OK). Whole module needs rewrite.**
...
***Let me also verify BUG-25 carefully by reading the V763 code:**
...
1. ***PMTP V763 is completely broken: begin\_write returns published slot, commit ignores slot, validate\_read always OK. Entire module needs rewrite.**

2. ***Dart ABI mismatch: `PolydimReport` in Dart has 9 fields, C has 11 (V763). Corruption on every FFI call.**
...
11. ***`polydim\_report\_init` doesn't initialize struct\_size/abi\_version in V763.**
...
- ***BUG-1, BUG-2 (PMTP V763: `slot` no se usa en commit; `validate\_read` devuelve siempre SUCCESS)**

- ***BUG-4 (Dart `PolydimReport` tiene 9 campos, C V763 tiene 11)**
...
- ***BUG-3: `(s \>\> 4) & 3` nunca produce 3 en V763 porque el estado inicial es `0x24` y las rotaciones solo mueven valores 0,1,2. No hay OOB.**
...
# 🛠️ ***FIX 1 — PMTP V763: reescritura completa**

***El PMTP V763 está roto en tres puntos independientes. No tiene sentido parchear; reescribo el módulo.**
 ...
 ***\* Bugs corregidos respecto de V763:**
...
***Este test falla con V763 (porque `validate\_read` siempre da OK y `races==0`, y porque `begin\_write` devuelve un slot fijo).**
...
```
***/\* Espejo EXACTO de PolydimReport en polydim.h V763.**
...
***extern "C" POLYDIM\_EXPORT void POLYDIM\_CALL polydim\_report\_init(PolydimReport\* r) \{**
    ...
    ***/\* V763: los campos nuevos deben inicializarse, no dejarse con basura. \*/**
...
## ***`polydim\_triton\_kernel\_v763.py` (fragmento corregido)**
...
***):**
    ...
    ***\# V763: yy incluido. Sin yy, la compuerta A4 no se puede aplicar en GPU.**
...
| ***7 Triton yy + no sync** | ***`polydim\_triton\_kernel\_v763.py`** | ***~15** | ***nuevo test** |
...
- ***BUG-3 (slot 3 inalcanzable en V763)**

- ***BUG-25 (Parche B es matemáticamente correcto)**

- ***BUG-18 (1 malloc, no 5)**

***¿Por qué 500 interacciones? Porque estuve auditando por categorías conceptuales SOTA en lugar de leer el código linealmente y verificar cada afirmación contra el binario. Los tres bugs críticos del PMTP V763 se ven a simple vista leyendo 40 líneas seguidas. No requerían conocimiento SOTA; requerían leer.**
...
***Wait, the V762 kernel in the first monolith has correct PMTP, but then later there's a V763 `polydim\_kernel.cpp` that has the broken PMTP. Let me check.**

***Yes, looking at the last polydim\_kernel.cpp (V763), the PMTP is broken:**
...
***That's a clear regression from V762 to V763.**
...
- ***`PolydimReport` in Dart V762 has 9 fields (8 doubles + 1 uint64), no struct\_size/abi\_version. If it's used against V763 C library → ABI mismatch.**
...
- ***The monolith V763 fixes these but V762 doesn't.**
...
***Wait, the V763 test\_suite.cpp has a bug:**
...
***This always passes. And if the V763 kernel's `validate\_read` always returns SUCCESS, then `races` counter would be 0, but the test still passes.**

***Also V763 test\_suite doesn't have a test that verifies the PMTP actually works correctly.**

***bench.cpp:**

- ***Only reports ortho\_err from V762 Stiefel (not V763)**
...
- ***Doesn't include the new V763 files (polydim\_abi.cpp, polydim\_pmtp.cpp, polydim\_arena.cpp)**
...
- ***The `PolydimStatus` enum in Dart doesn't have the same names as C (`errSubnormalDetected` = -4 in Dart file `polydim\_ffi\_v763.dart.txt`, but C has `DEGENERATE\_NORM = -4`). Different semantics.**

- ***Actually the V763 dart file uses different names entirely: `errTopologyFragmented(-6)` vs C's `POLYDIM\_ERR\_SEQLOCK\_RACE (-6)`. CONFLICT: same code, different names**

***Let me also check the V763 `polydim\_ffi\_v763.dart.txt`:**
...
2. ***Bugs in V762 → V763 regression**
...
***BUG: V763 PMTP V763 struct is only 1 byte.**
...
***`polydim\_pmtp\_sizeof()` returns 64 (aligned). `polydim\_pmtp\_init` stores 0x24. `polydim\_pmtp\_begin\_write` reads top 2 bits for slot. `commit\_write` rotates state. `validate\_read` returns SUCCESS always. So the V763 test would pass trivially.**

***Test in V763 test\_suite.cpp:**
...
***BUG: V763 `test\_suite.cpp` slot 3 OOB.**
...
***BUG: V763 `polydim\_pmtp\_begin\_write` returns slot 2 for the first call, but the reader reads slot 2 in `acquire\_read`:**
...
***So in the V763 the reader reads slot 1 while the writer writes to slot 2. The initial slot 1 has data 0. Reader thinks it has read something but reads garbage. Actually wait, `polydim\_pmtp\_acquire\_read` returns 1 with `\*slot\_out = middle = 1`, but the writer wrote to slot 2. Then `validate\_read` returns SUCCESS. So the reader will read slot 1 which contains whatever the test initialized it to (0). The test then verifies "all elements equal first" — since slot 1 was initialized to 0, all elements are 0, so `torn = false`. Test passes falsely.**
...
***In V763: `polydim\_pmtp\_acquire\_read` returns `middle`, not `newest`. So reader reads stale data.**
...
***OK, so V763 PMTP has a completely broken state machine. Confirmed BUG.**
...
***Now let me also check the `test\_v763\_mpeleides.py`:**
...
***So -4. Test expects -8 (DEGENERATE\_NORM in the v763 Dart file). MISMATCH.**

***So the test file `test\_v763\_mpeleides.py` expects codes that don't match what the Rust actually returns.**
...
- ***test\_v763\_mpeleides.py will fail on Suite 4 (Betti-1, function doesn't exist)**

- ***test\_v763\_mpeleides.py will fail on Suite 5 Attack 3 (expected -8 but Rust returns -4)**

- ***test\_v763\_mpeleides.py will fail on Suite 5 Attack 4 (expected -4 but Rust returns SUCCESS)**

- ***test\_v763\_mpeleides.py compiles kernel\_cpp\_v762.cpp but filename in repo is polydim\_kernel.cpp**

***Let me also check test\_v763\_mpeleides.py Suite 1:**
...
***The V763 monolith fixes this (5 args), but the V762 monolith and the test\_v763\_mpeleides.py both use the wrong 3-arg signature.**
...
2. ***Errores de regresión V762 → V763**
...
***Also, the `bench.cpp` includes `polydim.h` but calls `polydim\_stiefel\_cayley\_smw\_f64` (V762) not V763. So it uses V762 behavior. Not a bug per se but inconsistent with the V763 kernel.**

***One more: the V763 `polydim\_kernel.cpp` header comment says "POLYDIM V763" but the file in the "02\_ALL\_SOURCE\_SCRIPTS\_MONOLITH.md" section 2 is labeled V762. The V763 file appears separately. This is confusing naming.**
...
9. ***No `\_\_restrict\_\_` portability macro (V763 adds it, V762 has bare `\_\_restrict\_\_`? Actually V762 doesn't have it, so N/A)**

10. ***Test cota 2.10e-14 doesn't match kernel's internal 1.42e-14**

***B. Bugs in V762 → V763 regression (in V763 `polydim\_kernel.cpp`):**
...
15. ***V763 test `check(true, ...)` always passes**

16. ***V763 test doesn't exercise PMTP correctness**

17. ***V763 PMTP slot 3 latent even if currently unreachable**

***C. Bugs in FFI/ABI (Dart/Rust):**

18. ***V762 `PolydimReport` has 9 fields, C has 11 (V763)**

19. ***Dart never verifies ABI at runtime**

20. ***`polydim\_ffi\_v763.dart.txt` also has 3-arg Rust signature**
...
***G. Bugs in test\_v763\_mpeleides.py:**

46. ***Compiles `kernel\_cpp\_v762.cpp` (filename mismatch)**

47. ***Doesn't compile new V763 files (`polydim\_pmtp.cpp`, etc.)**
...
63. ***V762 vs V763 naming confusion in file headers**

64. ***Dart filename `polydim\_ffi.dart` vs `polydim\_ffi\_v763.dart.txt` — inconsistent packaging**
...
***A10. `polydim\_selftest\_all` no cubre la PMTP. Si la PMTP está rota (como en V763), el selftest sigue devolviendo SUCCESS.**
...
## ***B. 🔴 BUGS EN EL KERNEL V763 (`polydim\_kernel.cpp`)**
...
***B9. `polydim\_pmtp\_init` en V763 no inicializa `struct\_size` ni `abi\_version`. Comentario en el header dice que existen, pero ningún código los escribe.**

***B10. Los headers `polydim.h` V763 declaran `PolydimReport` con `struct\_size` y `abi\_version`, pero `polydim\_report\_init` en V763 no los setea. El cliente que llama `POLYDIM\_REPORT\_INIT` los inicializa, pero el kernel no los valida antes de escribir en el struct.**

***B11. No existe `polydim\_validate\_report(const PolydimReport\*)`. Un cliente compilado contra V762 (sin `struct\_size`) y enlazado contra V763 (con `struct\_size`) no detecta el ABI mismatch.**


## ***C. 🔴 BUGS EN FFI / ABI (Dart, Rust, C++)**

***C1. `dart/polydim\_ffi.dart` define `PolydimReport` con 9 campos. La V763 de `polydim.h` tiene 11 campos (añade `struct\_size` y `abi\_version` al inicio). `sizeOf\<PolydimReport\>()` ≠ `sizeof(PolydimReport)`. Cada llamada escribe 8 bytes fuera del buffer reservado → corrupción de heap.**

***C2. `Polydim.open()` en Dart no verifica el ABI después de abrir la biblioteca. Si la biblioteca es V763 y el Dart fue compilado contra V762, el bug C1 se manifiesta silenciosamente.**

***C3. `polydim\_ffi\_v763.dart.txt` usa 3 argumentos para `polydim\_rust\_verify\_invariants`:**
...
***C4. En `polydim\_ffi\_v763.dart.txt`, el enum Dart dice `errSubnormalDetected(-4)` pero el C++ tiene `POLYDIM\_ERR\_DEGENERATE\_NORM = -4`. Mismo código, semántica distinta.**

***C5. En `polydim\_ffi\_v763.dart.txt`, `errTopologyFragmented(-6)` colisiona con el C `POLYDIM\_ERR\_SEQLOCK\_RACE = -6`. Dos significados para el mismo número.**

***C6. `polydim\_ffi\_v763.dart.txt` declara `rustBettiGuardNative` para `polydim\_rust\_betti1\_guard`. Ese símbolo no existe en `lib.rs`. `lookupFunction` falla en runtime.**
...
***E7. `test\_v763\_mpeleides.py` compila `kernel\_cpp\_v762.cpp` pero el archivo del dossier se llama `polydim\_kernel.cpp` / `kernel\_cpp\_v763.cpp.txt`. Filename mismatch.**

***E8. `test\_v763\_mpeleides.py` no incluye los archivos nuevos (`polydim\_pmtp.cpp`, `polydim\_abi.cpp`, `polydim\_arena.cpp`).**

***E9. `test\_v763\_mpeleides.py` Suite 4 llama `verify\_betti1` → `polydim\_rust\_betti1\_guard`. Símbolo inexistente. Crash.**

***E10. `test\_v763\_mpeleides.py` Suite 5 Attack 3 espera `-8` (DEGENERATE\_NORM en Dart V763) pero Rust retorna `-4`. Test falla.**

***E11. `test\_v763\_mpeleides.py` Suite 5 Attack 4 espera `-4` (SUBNORMAL\_DETECTED) pero Rust cuenta subnormales y retorna `SUCCESS`. Test falla.**

***E12. `test\_v763\_mpeleides.py` usa el monolith V762 (3-arg Rust). Segfault en Suite 1.**
...
***F9. El header del archivo del dossier dice "POLYDIM V762", pero el código interno tiene comentarios "V763". Nomenclatura inconsistente.**
...
***F11. El enum en `polydim.h` V763 se llama `PolydimStatus` con códigos -1 a -12. La V763 del kernel añade `-13` y `-14` en Rust y Dart, pero los añade inconsistentemente (Dart V762 no los tiene; Dart V763 tiene nombres distintos).**
...
***H1. El monolith C++ está en un solo archivo de ~800 LOC sin separación en módulos. La V763 añade `polydim\_pmtp.cpp`, `polydim\_abi.cpp`, `polydim\_arena.cpp` pero la separación es incompleta.**
...
***H6. `test\_v763\_mpeleides.py` compila con `-static-libgcc -static-libstdc++` en Windows pero el CI documentado usa flags distintas. Inconsistencia entre CI y test.**
